...
void matrix_multiply_ping_pong_optimized() {
    ...
    unsigned int remaining_rows = A_rows % block_size;  // 矩阵 A 剩余的行数
    unsigned int remaining_cols = B_cols % block_size;  // 矩阵 B 剩余的列数

    unsigned int last_block_A = 12;
    unsigned int last_block_B = 12;
    ...
    for (unsigned int i = 0; i < A_rows; i += block_size) {
        ...
        unsigned int current_block_A = (i + block_size <= A_rows) ? block_size : remaining_rows;

        for (unsigned int j = 0; j < B_cols; j += block_size) {

            unsigned int current_block_B = (j + block_size <= B_cols) ? block_size : remaining_cols;
            ...
            if (i == 0 && j == 0) {
                ...
                write_register(cdma0, BTT_OFFSET, current_block_A * A_cols * 4);
                ...
                write_register(cdma1, BTT_OFFSET, current_block_B * B_rows * 4);
            ...
            if (ping_pong_flag == 0) {
                ...
                write_register(dma0, BTT_OFFSET, last_block_A * A_cols * 4);
                ...
                write_register(dma1, BTT_OFFSET, last_block_B * B_rows * 4);
                ...
                write_register(cdma0, BTT_OFFSET, last_block_A * last_block_B * 4);
                write_register(cdma0, CTRL_OFFSET, 0x1);  // 启动 CDMA0

                write_register(cdma1, SRC_ADDR_OFFSET, Shared_Men_11);
                write_register(cdma1, DST_ADDR_OFFSET, B_global_addr);  
                write_register(cdma1, BTT_OFFSET, last_block_A * last_block_B * 4);
                write_register(cdma1, CTRL_OFFSET, 0x1);  // 启动 CDMA1

                // 循环等待 数据搬移与计算完成
                wait_for_interrupt();
                last_block_A = current_block_A;
                last_block_B = current_block_B;
                A_global_addr = A_global_addr + last_block_A * last_block_B * 4;
                B_global_addr = B_global_addr + last_block_A * last_block_B * 4;
